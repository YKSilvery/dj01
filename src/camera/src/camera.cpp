...
public:
  ...
  {
    image_publisher_ = this->create_publisher<aimbot_msgs::msg::CameraImage>(
      "camera_image", rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(rmw_qos_profile_sensor_data))
        .reliability(RMW_QOS_POLICY_RELIABILITY_BEST_EFFORT));

    std::filesystem::path video_dir;
    try {
      const auto share_path = ament_index_cpp::get_package_share_directory("camera");
      video_dir = std::filesystem::path(share_path) / "video";
    } catch (const std::exception & e) {
      const auto source_dir = std::filesystem::path(__FILE__).parent_path();
      const auto fallback_dir = (source_dir / "video").string();
      video_dir = fallback_dir;
      RCLCPP_WARN(
        this->get_logger(),
        "Falling back to source video directory '%s' (%s)",
        fallback_dir.c_str(),
        e.what());
    }
    ...
    if (std::filesystem::is_directory(video_dir)) {
      for (const auto & entry : std::filesystem::directory_iterator(video_dir)) {
        if (entry.is_regular_file()) {
          default_video = entry.path().string();
          break;
        }
      }
    }

    this->declare_parameter("video_directory", video_dir.string());
    this->declare_parameter("video_file", default_video);
    this->declare_parameter("publish_rate", 30.0);

    auto configured_video = this->get_parameter("video_file").as_string();
    if (configured_video.empty()) {
      const auto video_directory = this->get_parameter("video_directory").as_string();
      configured_video = (std::filesystem::path(video_directory) / "video_v1.avi").string();
    }

    if (!std::filesystem::exists(configured_video)) {
      RCLCPP_ERROR(this->get_logger(), "Video file not found: %s", configured_video.c_str());
      RCLCPP_ERROR(this->get_logger(), "Please place video files in src/camera/src/video/ and rebuild");
      throw std::runtime_error("Missing video source");
    }

    video_file_ = configured_video;
    if (!capture_.open(video_file_)) {
    ...
    const double desired_rate = std::max(this->get_parameter("publish_rate").as_double(), 1.0);
    const auto period = std::chrono::duration<double>(1.0 / desired_rate);
...
private:
  void publishFrame()
  {
    if (!capture_.isOpened()) {
      RCLCPP_WARN(this->get_logger(), "Video capture is not open");
      return;
    }

    if (!capture_.read(frame_) || frame_.empty()) {
      RCLCPP_DEBUG(this->get_logger(), "Restarting video playback from beginning");
      restartStream();
      if (!capture_.read(frame_) || frame_.empty()) {
        RCLCPP_ERROR(this->get_logger(), "Unable to read frames from video file: %s", video_file_.c_str());
        return;
      }
    }
    ...
    msg.height = static_cast<uint32_t>(frame_.rows);
    msg.width = static_cast<uint32_t>(frame_.cols);
    msg.encoding = "bgr8";
    msg.is_bigendian = false;
    msg.step = static_cast<uint32_t>(frame_.cols * frame_.channels());
    const size_t data_size = static_cast<size_t>(frame_.total() * frame_.elemSize());
    msg.data.resize(data_size);
    std::memcpy(msg.data.data(), frame_.data, data_size);

    aimbot_msgs::msg::CameraImage camera_msg;
    camera_msg.header = msg.header;
    camera_msg.timestamp = static_cast<uint64_t>(this->now().nanoseconds());
    camera_msg.image = std::move(msg);
  ...
  rclcpp::Publisher<aimbot_msgs::msg::CameraImage>::SharedPtr image_publisher_;
...
class CameraNode : public rclcpp::Node                 //别忘了相机畸变校正
{
public:
  ...
  static void __stdcall ImageCallback(unsigned char *pData, MV_FRAME_OUT_INFO_EX *pFrameInfo, void *pUser)
  {
    ...
    if (pData && pFrameInfo) {
      ...
      // 优化数据拷贝：预分配空间并使用memcpy进行高效拷贝
      image_msg.data.resize(pFrameInfo->nFrameLen);
      memcpy(image_msg.data.data(), pData, pFrameInfo->nFrameLen);

      // 创建CameraImage消息
      aimbot_msgs::msg::CameraImage camera_msg;
      camera_msg.header = image_msg.header;
      uint64_t timestamp = ((uint64_t)pFrameInfo->nDevTimeStampHigh << 32) | pFrameInfo->nDevTimeStampLow;
      camera_msg.timestamp = timestamp;
      camera_msg.image = std::move(image_msg);

      node->image_publisher_->publish(camera_msg);

      // 优化日志输出：减少频率，避免每次帧都输出
      ...
      if (frame_count % 100 == 0) {  // 每100帧输出一次
        ...
        RCLCPP_INFO(node->get_logger(), "Published frame %d: %dx%d, PixelType: %ld, Encoding: %s",
            frame_count, pFrameInfo->nWidth, pFrameInfo->nHeight, pFrameInfo->enPixelType,
  ...
  : Node("camera_node", options), handle_(nullptr), is_grabbing_(false), is_reconnecting_(false), payload_size_(0)
  {
    // 创建图像发布者 - 增加队列大小以提高性能
    image_publisher_ = this->create_publisher<aimbot_msgs::msg::CameraImage>(
      "camera_image", rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(rmw_qos_profile_sensor_data))
    ...
    this->declare_parameter("gain", 20.0);  // 默认20dB
    this->declare_parameter("pixel_format", std::string("BayerRG8"));  // 默认BayerRG8
        ...
        if (stDeviceList.nDeviceNum > 0) {
          ...
          if (create_result == MV_OK) {
            ...
            if (open_result == MV_OK) {
              ...
              // 设置缓存节点个数 - 增加到3以提高性能
              int node_result = MV_CC_SetImageNodeNum(handle_, 3);
...
int main(int argc, char **argv)
{
  ...
  auto node = std::make_shared<camera::CameraNode>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}